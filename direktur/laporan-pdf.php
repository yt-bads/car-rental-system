<?php
// direktur/laporan-pdf.php
// Generator PDF Laporan - Direktur — Rental Mobil PT. Wildan Abadi Jaya
...
<body>
    <div class="header">
        ...
        <p class="meta">Dicetak oleh Direktur: ' . htmlspecialchars($_SESSION['nama']) . ' | ' . date('d M Y H:i') . '</p>
    </div>
    <hr />
...
try {
    ...
    $mpdf->SetTitle('Laporan Rental PT. Wildan Abadi Jaya (Direktur)');
    $mpdf->WriteHTML($html);
    
    // Set output name
    $filename = 'Laporan-Rental-Direktur-' . date('d-m-Y', strtotime($dari)) . '-sd-' . date('d-m-Y', strtotime($sampai)) . '.pdf';
